...
const DiningByTime: React.FC = () => {
  ...
  return (
    <section className="py-32 bg-[#08210c] overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-wrap justify-center gap-4 md:gap-0 border-b border-white/5 mb-20">
          {TIME_SLOTS.map((slot) => (
            <button
              key={slot.id}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <div className="relative group">
            <div className="absolute -inset-4 border border-accent/20 -z-10 translate-x-2 translate-y-2 group-hover:translate-x-0 group-hover:translate-y-0 transition-transform duration-500"></div>
            <div className="overflow-hidden aspect-[4/5] shadow-2xl relative">
              <div className="absolute inset-0 bg-gradient-to-t from-[#051408] via-transparent to-transparent z-10" />
              <img 
          ...
          <div className="animate-fade-in p-8" key={activeSlot.id}>
            <div className="inline-block px-4 py-1 border border-accent/40 rounded-full mb-6">
                <span className="text-accent text-[10px] font-bold uppercase tracking-widest">Selected Curations</span>
            </div>
            <h3 className="text-5xl font-serif text-white mb-6 italic">{activeSlot.title}</h3>
            <p className="text-stone-400 mb-12 font-light text-lg leading-relaxed border-l-2 border-accent/30 pl-8 italic">
              ...
              {activeSlot.dishes.map((dish, idx) => (
                <div key={idx} className="group cursor-default">
                  <div className="flex justify-between items-end mb-2">
                    <span className="text-xl font-serif text-stone-200 group-hover:text-accent transition-colors">{dish}</span>
                    <div className="flex-1 border-b border-dotted border-white/10 mx-4 mb-1"></div>
                    <span className="text-accent font-serif italic text-sm">Signature</span>
                  </div>
                </div>
              ))}
            </div>
